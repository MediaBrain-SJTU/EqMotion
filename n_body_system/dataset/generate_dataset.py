...
parser.add_argument('--initial_loc', type=float, default=1.,
                    help='consider initial velocity')
parser.add_argument('--initial_vel_n', type=float, default=0.5,
                    help='consider initial velocity')

args = parser.parse_args()

initial_vel_norm = args.initial_vel_n
...
if args.simulation == 'springs':
    ...
    sim = ChargedParticlesSim(noise_var=0.0, n_balls=args.n_balls, vel_norm=initial_vel_norm, loc_std=args.initial_loc)
    suffix = '_charged'
elif args.simulation == 'charged2':
    sim = ChargedParticlesSim(noise_var=0.0, n_balls=args.n_balls, vel_norm=initial_vel_norm, interaction_strength=1)
    suffix = '_charged_pos'
else:
...
def generate_dataset(num_sims, length, sample_freq):
    ...
    for i in range(num_sims):
        t = time.time()
        if args.simulation == 'charged2':
            loc, vel, edges, charges = sim.sample_trajectory(T=length,
                                                    sample_freq=sample_freq,charge_prob=[1./3, 1. / 3, 1. / 3])
        else:
            loc, vel, edges, charges = sim.sample_trajectory(T=length,
...
if __name__ == "__main__":
    ...
    if args.simulation == 'charged' or args.simulation == 'charged2':
